...
def save_mail(mail: List[Dict[str, str]]) -> None:
    """
    Saves all mail to a json file
    
    Args:
        mail: a list of mail entries
    """
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    ...
    Args:
        mail_entry: one mail entry that was sent
...
def delete_mail(mail_id: str) -> bool:
    ...
    Args:
       mail_id: The id of the mail entry to be deleted

    Returns:
        bool: True if the desired mail entry was delted. False otherwise
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    """
    Fetches a mail entry from the json file and displays its contents

    Args:
        mail_id: The id of the mail entry to be retrieved

    Returns:
        Optional Dict: Contents of the mail entry
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """
    Fetches a user's inbox from the json file

    Args:
        recipient: The desired user's inbox

    Returns:
        List: A list of mail entries
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """
    Fetches all mail sent by a given person from the json file

    Args:
        sender: The desired sender to find mail from

    Returns:
        List: A list of mail sent from the sender
    """
    mail = load_mail()
    sent = []
    for entry in mail:
...
def get_sender_route(sender: str):
    """
    Summary: Gets all mail entried from a sender from the json file

    Args:
        sender (str): The sender of the mail
    Returns:
        list: A list of dictionaries representing the mail entries from the sender
    """
